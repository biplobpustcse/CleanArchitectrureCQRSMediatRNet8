...
namespace CleanArchitectrure.Persistence.Repositories
{
    public class CustomerRepository : ICustomerRepository
    {
        ...
        {
            using var connection = _applicationContext.CreateConnection();
            var query = "SELECT * FROM dbo.Customers WHERE CustomerID='" + id + "' ";

            var customers = await connection.QuerySingleOrDefaultAsync<Customer>(query, commandType: CommandType.Text);
            return customers;
        }
        #endregion
